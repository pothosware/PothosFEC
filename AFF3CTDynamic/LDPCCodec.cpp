...
LDPCCodec<B,Q>
::LDPCCodec(const factory::Encoder_LDPC  ::parameters &enc_params,
...
{
	const std::string name = "LDPCCodec";
	this->set_name(name);
	...
	if (info_bits_pos.empty())
	{
		if (enc_params.type == "LDPC_H")
			this->set_encoder(LDPCEncoderFactory::build<B>(enc_params, G, H));
	}
	else
	{
	...
	{ // encoder not set when building encoder LDPC_H
		try
		{
			this->set_encoder(LDPCEncoderFactory::build<B>(enc_params, G, H, *dvbs2));
		}
		catch(tools::cannot_allocate const&)
		{
	...
	try
	{
		this->set_decoder_siso_siho(LDPCDecoderFactory::build_siso<B,Q>(dec_params, H, info_bits_pos, this->get_encoder()));
	}
	catch (const std::exception&)
	{
		this->set_decoder_siho(LDPCDecoderFactory::build<B,Q>(dec_params, H, info_bits_pos, this->get_encoder()));
	}
}

template <typename B, typename Q>
void LDPCCodec<B,Q>
::_extract_sys_par(const Q *Y_N, Q *sys, Q *par, const int)
{
	const auto K    = this->K;
...
template <typename B, typename Q>
void LDPCCodec<B,Q>
::_extract_sys_llr(const Q *Y_N, Q *Y_K, const int)
{
...
template <typename B, typename Q>
void LDPCCodec<B,Q>
::_extract_sys_bit(const Q *Y_N, B *V_K, const int)
{
	for (auto i = 0; i < this->K; i++)
...
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::LDPCCodec<B_8,Q_8>;
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::LDPCCodec<B_16,Q_16>;
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::LDPCCodec<B_32,Q_32>;
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::LDPCCodec<B_64,Q_64>;
// ==================================================================================== explicit template instantiation
