...
LDPCMatrixHandler::Matrix_format LDPCMatrixHandler
...
Sparse_matrix LDPCMatrixHandler
...
::transform_H_to_G_decomp_LU(const Sparse_matrix& H, Positions_vector& info_bits_pos)
{
	...
	auto Gf = LDPCMatrixHandler::transform_H_to_G_decomp_LU(Hf, info_bits_pos);
...
::transform_H_to_G_identity(const Sparse_matrix& H, Positions_vector& info_bits_pos)
{
	...
	auto Gf = LDPCMatrixHandler::transform_H_to_G_identity(Hf, info_bits_pos);

	return full_to_sparse(Gf);
}

void swap_columns(LDPCMatrixHandler::LDPC_matrix& mat, size_t idx1, size_t idx2)
{
	auto n_row = mat.get_n_rows();
	std::vector<LDPCMatrixHandler::LDPC_matrix::value_type> tmp(n_row);
...
LDPCMatrixHandler::LDPC_matrix LU_decomp(const LDPCMatrixHandler::LDPC_matrix& Hp)
{
	using V = LDPCMatrixHandler::LDPC_matrix::value_type;

	Hp.is_of_way_throw(Matrix::Way::HORIZONTAL);

	auto M = Hp.get_n_rows();

	LDPCMatrixHandler::LDPC_matrix Hinv = Hp.resize(M, 2 * M, Matrix::Origin::TOP_LEFT);
...
LDPCMatrixHandler::LDPC_matrix LU_decomp2(const LDPCMatrixHandler::LDPC_matrix& Hp)
{
	using V = LDPCMatrixHandler::LDPC_matrix::value_type;
	...
	LDPCMatrixHandler::Positions_vector null_rows;
	...
	LDPCMatrixHandler::Positions_vector pos_rows(n_rows), pos_cols(n_cols);
...
LDPCMatrixHandler::LDPC_matrix LDPCMatrixHandler
...
::transform_H_to_G_identity(const LDPC_matrix& H, Positions_vector& info_bits_pos)
{
	...
	auto swapped_cols = LDPCMatrixHandler::form_diagonal(G, Matrix::Origin::TOP_LEFT);
	LDPCMatrixHandler::form_identity(G, Matrix::Origin::TOP_LEFT);
...
::transform_H_to_G_identity(const LDPC_matrix& H, Positions_vector& info_bits_pos)
{
	...
	auto swapped_cols = LDPCMatrixHandler::form_diagonal(G, Matrix::Origin::BOTTOM_RIGHT);
	LDPCMatrixHandler::form_identity(G, Matrix::Origin::BOTTOM_RIGHT);
...
LDPCMatrixHandler::Positions_pair_vector LDPCMatrixHandler
...
LDPCMatrixHandler::Positions_vector LDPCMatrixHandler
