...
RepetitionDecoder<B,R>
::RepetitionDecoder(const int& K, const int& N, const bool buffered_encoding, const int n_frames)
: Decoder               (K, N, n_frames, 1),
  Decoder_SISO_SIHO<B,R>(K, N, n_frames, 1),
  rep_count((N/K) -1), buffered_encoding(buffered_encoding), sys(K), par(K * rep_count), ext(K)
{
	const std::string name = "RepetitionDecoder";
	this->set_name(name);
...
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::RepetitionDecoder<B_8,Q_8>;
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::RepetitionDecoder<B_16,Q_16>;
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::RepetitionDecoder<B_32,Q_32>;
template class AFF3CTDynamic::POTHOS_SIMD_NAMESPACE::RepetitionDecoder<B_64,Q_64>;
// ==================================================================================== explicit template instantiation
