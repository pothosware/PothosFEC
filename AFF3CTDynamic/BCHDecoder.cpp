...
BCHDecoder<B,R>
::BCHDecoder(const int& K, const int& N, const aff3ct::tools::BCH_polynomial_generator<B> &GF_poly, const int n_frames)
: aff3ct::module::Decoder         (K, N,                  n_frames, mipp::N<B>()           ),
  aff3ct::module::Decoder_BCH<B,R>(K, N, GF_poly.get_t(), n_frames                         ),
  t2              (2 * this->t                                             ),
...
{
	const std::string name = "BCHDecoder";
	this->set_name(name);
